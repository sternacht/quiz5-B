...
bool list_contains(hp_t **head, uintptr_t ptr) {
  ...
  LIST_ITER(head, node) {
    if (node->active_ == true && atomic_load(&node->ptr) == ptr)
      return true;
  }
...
static void cleanup_ptr(domain_t *dom, wconfig_t *wconfig, uintptr_t ptr,
                        int flags) {
 ...
 */
void swap(domain_t *dom, wconfig_t *wconfig, uintptr_t *prot_ptr,
          uintptr_t new_val, int flags) {
